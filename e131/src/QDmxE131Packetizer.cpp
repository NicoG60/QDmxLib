...
QDmxE131Packetizer::QDmxE131Packetizer()
{
    ...
    QString sourceName("QDmxLib - E1.31");
    m_commonHeader.append(sourceName.leftJustified(64, '\0', true));
...
void QDmxE131Packetizer::setupE131Dmx(QByteArray& data, const int &universe, const int &priority, const QByteArray &values)
{
    data.clear();
...
bool QDmxE131Packetizer::checkPacket(QByteArray &data)
{
...
bool QDmxE131Packetizer::fillDMXdata(QByteArray& data, QByteArray &dmx, quint32 &universe)
{
    if (data.isNull())
        return false;
    dmx.clear();
