...
QDmxArtNetPacketizer::QDmxArtNetPacketizer()
{
...
void QDmxArtNetPacketizer::setupArtNetPoll(QByteArray& data)
{
    data.clear();
...
void QDmxArtNetPacketizer::setupArtNetPollReply(QByteArray &data, ArtNetNodeInfo &info, QHostAddress ipAddr, QString MACaddr)
{
    QByteArray shortName = info.shortName.toLatin1();
    QByteArray longName = info.longName.toLatin1();
    shortName = shortName.leftJustified(18, 0, true);
    longName = longName.leftJustified(64, 0, true);
    int i = 0;
    data.clear();
    data.append(m_commonHeader);
    data.remove(9, 2);
    ...
    data.append(shortName);   // Short Name
    data.append(longName); // Long Name
    for (i = 0; i < 64; i++)
    ...
    data.append((char)0x04);     // NumPort LSB
    data.append((char)0xC0);     // Port 1 type: can output DMX512 data
    data.append((char)0xC0);     // Port 2 type: can output DMX512 data
    data.append((char)0xC0);     // Port 3 type: can output DMX512 data
    data.append((char)0xC0);     // Port 4 type: can output DMX512 data
    // FIXME: this should reflect the actual state of QLC+ output ports !
    for (i = 0; i < 8; i++)
        data.append((char)0x00); // Set GoodInput[4] and GoodOutput[4] all to unknown state
    data.append((char)0x00);     // SwIn0 - input 0
    data.append((char)0x01);     // SwIn1 - input 1
    data.append((char)0x02);     // SwIn2 - input 2
    data.append((char)0x03);     // SwIn3 - input 3
...
void QDmxArtNetPacketizer::setupArtNetDmx(QByteArray& data, const int &universe, const QByteArray &values)
{
    data.clear();
    data.append(m_commonHeader);
...
bool QDmxArtNetPacketizer::checkPacketAndCode(QByteArray const& data, int &code)
{
...
bool QDmxArtNetPacketizer::fillArtPollReplyInfo(QByteArray const& data, ArtNetNodeInfo& info)
{
    if (data.isNull())
        return false;
...
bool QDmxArtNetPacketizer::fillDMXdata(QByteArray const& data, QByteArray &dmx, quint32 &universe)
{
    if (data.isNull())
        return false;
    dmx.clear();
